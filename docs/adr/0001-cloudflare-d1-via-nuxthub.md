...
- Prefer an explicit Drizzle terminal method on writes (`.returning()`, `.run()`, `.get()`, or `.all()`) so the D1 driver actually invokes `stmt.run` / `stmt.raw`.
- **Do not send a JSON body on `DELETE`.** Nitro `readBody` on DELETE-with-body hangs on Cloudflare Workers (Error 1101 / “script will never generate a response”) while the same handler works locally against libSQL. Uncheck uses `DELETE /api/completions/:choreId/:dayOfWeek` (path params), matching assignment deletes — not `DELETE /api/completions` with a body. Confirmed in the wild on [workerd#210](https://github.com/cloudflare/workerd/issues/210) (Nuxt/Nitro + DELETE body).
- Local NuxtHub API tests run against libSQL and do not exercise Cloudflare’s DELETE-body hang or Drizzle’s D1 batch driver. D1/Workers-specific write patterns need focused regression coverage (or a production smoke check) in addition to normal API behavior tests.
