...
**Rain check**:
The record that an active Chore is sitting out for one specific Week — still on the board for that Week, but muted and not checkable, without archiving the Chore or changing its Assignments. One rain check covers every Assignment for that Chore in that Week (not per day bucket); each of those day buckets shows the muted row. Completions for that Chore/Week may already exist when a rain check is taken; they stay as history (and as sparkles), but the board stays muted and not checkable until the rain check is cleared — new Completions are not allowed while rain-checked. Clearing a rain check removes only that Week’s rain-check record; Completions are untouched. Past Weeks’ rain checks are kept as history (no auto-expire); the next Week shows the Chore as normal unless another rain check is taken. Does not apply to archived Chores.
_Avoid_: Skip, pause, snooze, hide, defer, mute, temporary archive, off-week (as automation); wiping Completions on take or clear
