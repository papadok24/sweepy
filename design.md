...
- **Opened Chore (Notes + List)** — peer **Details | List** tabs inside the opened-Chore drawer (prototype [#74](https://github.com/papadok24/sweepy/issues/74) variant C). Details: name / days / multi-line plain Notes. List: grocery-scale plain-text add/remove (no check-off). Light list cue (icon + count) on **Today** rows when the List has items — not on Week. Not push-stack or nested-sheet navigation. Prototype branch `prototype/74-opened-chore-notes-list`.
